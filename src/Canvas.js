  ...
  /** @lends draw2d.Canvas.prototype */
  {
    ...
    init: function (canvasId, width, height) {
      ...
      $(this.html).droppable({
        ...
        drop: function drop(event, ui) {
          ...
          let pos = _this.fromDocumentToCanvasCoordinate(
            event.clientX,
            event.clientY
          );
          _this.onDrop(
            ...
            pos.getY() - (event.clientY - helperPos.top) + 5,
            event.shiftKey,
            event.ctrlKey
          );
        },
      ...
      this.regionDragDropConstraint = new draw2d.policy.figure.RegionEditPolicy(
        0,
        0,
        this.getWidth(),
        this.getHeight()
      ...
      ); // Responsible for selection handling
      ...
      this.installEditPolicy(
        new draw2d.policy.connection.ComposedConnectionCreatePolicy([
          // Responsible for connection creation....
      ...
      this.html.bind("mousemove touchmove", function (event) {
        ...
        if (_this.mouseDown === false) {
          ...
          try {
            ...
            if (
              hover !== _this.currentHoverFigure &&
              ...
              _this.fireEvent("mouseleave", {
                figure: _this.currentHoverFigure,
              });
            }
      ...
      this.html.on(
        "MozMousePixelScroll DOMMouseScroll mousewheel",
        function (e) {
          ...
          let returnValue = _this.onMouseWheel(
            delta,
            pos.x,
            pos.y,
            event.shiftKey,
      ...
      while (lines.getSize() > 0) {
        ...
        lines.each((ii, l2) => {
          ...
          if (partInter.getSize() > 0) {
            this.lineIntersections.add({
              line: l1,
              other: l2,
              intersection: partInter,
            });
            this.lineIntersections.add({
              line: l2,
              other: l1,
    ...
    installEditPolicy: function (policy) {
      ...
      //
        ...
        policy instanceof draw2d.policy.connection.ConnectionCreatePolicy
      ) {
        this.editPolicy.grep((p) => {
          let stay = !(
            p instanceof draw2d.policy.connection.ConnectionCreatePolicy
          );
          if (stay === false) {
            p.onUninstall(this);
          }
    ...
    setDimension: function (dim, height) {
      if (typeof dim === "undefined") {
        let widths = this.getFigures()
          .clone()
          .map(function (f) {
            return f.getAbsoluteX() + f.getWidth();
          });
        let heights = this.getFigures()
          .clone()
          .map(function (f) {
    ...
    addSelection: function (object) {
      ...
      if (
        object instanceof draw2d.util.ArrayList ||
        object instanceof draw2d.Selection
      ) {
        object.each(add);
      } else {
        add(0, object);
    ...
    getBestFigure: function (x, y, blacklist, whitelist) {
      ...
      let isInList = function (testFigure, list) {
        for (let i = 0, len = list.length; i < len; i++) {
          ...
          if (typeof considering === "function") {
            ...
            considering === testFigure ||
            considering.contains(testFigure)
          ) {
            return true;
          }
        }
      ...
      for (let i = 0, len = this.resizeHandles.getSize(); i < len; i++) {
        ...
        if (
          testFigure.isVisible() &&
          testFigure.hitTest(x, y) &&
          !isInBlacklist(testFigure) &&
          isInWhitelist(testFigure)
        ) {
          return testFigure;
        }
      }
      ...
      for (let i = this.figures.getSize() - 1; i >= 0; i--) {
        ...
        if (
          ...
          figure.isVisible() &&
          figure.hitTest(x, y) &&
          !isInBlacklist(figure) &&
          isInWhitelist(figure)
        ) {
          result = figure;
          break;
        }
      ...
      let figureIndex =
        figureResult !== null ? $(figureResult.shape.node).index() : -1;
      let childIndex =
        childResult !== null ? $(childResult.shape.node).index() : -1;
      let lineIndex =
        lineResult !== null ? $(lineResult.shape.node).index() : -1;
      let array = [
    ...
    getBestLine: function (x, y, lineToIgnore) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (
          line.isVisible() === true &&
          line.hitTest(x, y) === true &&
          $.inArray(line, lineToIgnore) === -1
        ) {
          return line;
        }
      }
    ...
    onMouseWheel: function (wheelDelta, x, y, shiftKey, ctrlKey) {
      ...
      this.editPolicy.each(function (i, policy) {
        returnValue =
          policy.onMouseWheel(wheelDelta, x, y, shiftKey, ctrlKey) &&
          returnValue;
      });
    ...
    off: function (eventOrFunction) {
      if (typeof eventOrFunction === "undefined") {
        ...
        for (let event in this.eventSubscriptions) {
          this.eventSubscriptions[event] = this.eventSubscriptions[
            event
